...
@IonicPage()
@Component({
  selector: 'page-stock-outdetail',
  templateUrl: 'stock-outdetail.html',
})
export class StockOutdetailPage {
  tests: any;
  ...
  site:String;
  ionViewDidLoad() {
    console.log('ionViewDidLoad StockOutdetailPage');
  ...
  confirmForm(){
    this.Service.PostStockOrder(this.number,this.date,this.site,this.status,this.username).subscribe((data)=>{ 
      if(data){
        ...
        this.StockOutFormSuccess();
        this.navCtrl.push(MainPage);
        
      }else{
        this.StockOutFormFail();
      }
